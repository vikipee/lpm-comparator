...
const LPMCard = ({
  ...
  setSelectedSide: (side: 1 | 2) => void;
  setSelectedLpm: (selectedLpm: LocalProcessModel) => void;
}) => {
  ...
  const [filterValues, setFilterValues] = useState<
    Record<Exclude<LpmSortOption, 'name'>, [number, number]>
  >(defaultLpmFilterValues);
  ...
  const sortedAndFilteredLpms = sortAndFilterLpms(
    lpms,
    filterValues,
    sortBy,
    sortOrder,
  ...
  return (
    <Card
      className="h-[calc(100vh-8rem)] flex flex-col mt-2"
      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex space-x-2">
          <GenericSortPopover
            ...
            sortBy={sortBy}
            sortOrder={sortOrder}
            options={[['name'], ['fitness'], ['precision'], ['coverage']]}
          />
          <GenericFilterPopover
            ...
            resetFilters={() => setFilterValues(defaultLpmFilterValues)}
            options={[['fitness'], ['precision'], ['coverage']]}
          />
        </div>
      </CardHeader>
          ...
          {paginatedLpms.map((lpm) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">
                F: {lpm.fitness.toFixed(2)}, P: {lpm.precision.toFixed(2)}, C:{' '}
                {lpm.coverage.toFixed(2)}
              </span>
            </li>
          ))}
        </ul>
...
export default function LpmList({ report }: { report: ReportData }) {
  const [selectedLpm, setSelectedLpm] = useState<LocalProcessModel | null>(
    null,
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <LPMDialog
        side={selectedSide}
        setSide={setSelectedSide}
        selectedLpm={selectedLpm}
        setSelectedLpm={setSelectedLpm}
        report={report}
      />
    </div>
