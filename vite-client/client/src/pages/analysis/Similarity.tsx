...
export default function Similarity({ report }: { report: ReportData }) {
  ...
  const sortedLpmsA = sortAndFilterLpms(
    report.lpms_a,
    filterValues,
    sortBy,
    sortOrder,
  ).slice(0, 15);
  const sortedLpmsB = sortAndFilterLpms(
    report.lpms_b,
    filterValues,
    sortBy,
    sortOrder,
      ...
      <CardContent className="h-[calc(100vh-8rem)] overflow-auto">
        {noLPMs ? (
          <div className="text-center h-full flex-col">
            {' '}
            <h3 className="my-auto font-bold">No LPMs to display</h3>{' '}
            <p>Change your filters to see the Heatmap again</p>
          </div>
        ) : (
