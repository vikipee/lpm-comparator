...
export function EulerDiagram({report}: {report: ReportData}) {
    ...
    useEffect(() => {
        if (chartRef.current) {
            ...
            const {numSetA, numIntersection, numSetB} = getDataForVennDiagram(similarityMatrix, threshold);
            ...
            const chart = new Chart(chartRef.current, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['hsl(181, 97%, 38%)', 'hsl(37, 94%, 54%)', 'hsl(104.78 29.78% 55.88%)'],
                    }]
                },
                options: {
                    responsive: true,
    ...
    return (
                ...
                <div className="flex space-x-2">
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="flex flex-col items-center">
                        <span className="text-sm text-gray-500 mb-2">Threshold:{threshold.toFixed(2)}</span>
                        <Slider
                          min={0}
                          max={1}
                          step={0.01}
      ...
      <CardContent className="h-[calc(100vh-8rem)] overflow-hidden">
          { noData ? (<div className="text-center h-full flex-col"> 
          <h3 className="my-auto font-bold">No Data to display</h3></div>) : (<canvas ref={chartRef} className="max-h-full max-w-full"/>)}
      </CardContent>
  </Card>);
}