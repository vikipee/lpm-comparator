...
export const defaultLpmFilterValues: Record<
  Exclude<LpmSortOption, 'name'>,
  [number, number]
> = {
  fitness: [0, 1],
  precision: [0, 1],
...
export type TraceCoverageSortOption =
  | 'coverage_a'
  | 'duplicate_coverage_a'
  | 'trace'
  | 'coverage_b'
  | 'duplicate_coverage_b';

export const defaultTraceCoverageFilterValues: Record<
  Exclude<TraceCoverageSortOption, 'trace'>,
  [number, number]
> = {
  coverage_a: [0, 1],
  coverage_b: [0, 1],
...
export const sortAndFilterLpms = (
  lpms: LocalProcessModel[],
  filterValues: Record<Exclude<LpmSortOption, 'name'>, [number, number]>,
  sortBy: LpmSortOption,
  sortOrder: SortOrder,
) => {
  return lpms
    .filter(
      (lpm) =>
        lpm.fitness >= filterValues.fitness[0] &&
        lpm.fitness <= filterValues.fitness[1] &&
        lpm.precision >= filterValues.precision[0] &&
        lpm.precision <= filterValues.precision[1] &&
        lpm.coverage >= filterValues.coverage[0] &&
        lpm.coverage <= filterValues.coverage[1],
    )
    .sort((a, b) => {
      if (sortBy === 'name') {
...
export const sortAndFilterTraceCoverages = (
  traces: Trace[],
  filterValues: Record<
    Exclude<TraceCoverageSortOption, 'trace'>,
    [number, number]
  >,
  sortBy: TraceCoverageSortOption,
  sortOrder: SortOrder,
  variantsIdx: number[],
) => {
  ...
  const filteredTraces = traces.filter((trace) => {
    ...
    return (
      withinCoverageA &&
      withinCoverageB &&
      withinDuplicateCoverageA &&
      withinDuplicateCoverageB
    );
  });
  return filteredTraces.sort((a, b) => {
    if (sortBy === 'trace') {
      return sortOrder === 'asc'
        ? a.trace.localeCompare(b.trace)
        : b.trace.localeCompare(a.trace);
    } else if (sortBy === 'coverage_a') {
      return sortOrder === 'asc'
        ? a.coverage_a - b.coverage_a
        : b.coverage_a - a.coverage_a;
    } else if (sortBy === 'duplicate_coverage_a') {
      return sortOrder === 'asc'
        ? a.duplicate_coverage_a - b.duplicate_coverage_a
        : b.duplicate_coverage_a - a.duplicate_coverage_a;
    } else if (sortBy === 'duplicate_coverage_b') {
      return sortOrder === 'asc'
        ? a.duplicate_coverage_b - b.duplicate_coverage_b
        : b.duplicate_coverage_b - a.duplicate_coverage_b;
    } else {
      return sortOrder === 'asc'
        ? a.coverage_b - b.coverage_b
        : b.coverage_b - a.coverage_b;
    }
  });
};
            ...
            <div key={option[0]} className="flex items-center justify-between">
              ...
              <div>
                ...
                <Button
                  ...
                  className={
                    sortBy === option[0] && sortOrder === 'desc'
                      ? 'bg-accent'
                      : ''
                  }
                >
          ...
          {options.map((metric) => (
            <div key={metric[0]} className="space-y-2">
              ...
              <Slider
                ...
                onValueChange={(value) =>
                  onFilterChange(metric[0], value as [number, number])
                }
